...
[
  ...
  },
  {
    ...
    "help": "By default most Custom Variables should be set to 3 (Page)",
    "notSetText": ""
  },
  {
...
if (data.hasOwnProperty("cvars")) {
    for (var i = 0; i < data.cvars.length; i++) {
        ...
        let scope = data.cvars[i].scope;

        if (typeof scope === "undefined" || scope === "") {
            scope = 3;
        } else {
            scope = makeNumber(scope);
        }
...
function onSuccess() {
    ...
    for (var i = 0; i < cvarScope4Array.length; i++) {
        var slot = cvarScope4Array[i].slot;
        var key = cvarScope4Array[i].key;
        var value = cvarScope4Array[i].value;
        var scope = cvarScope4Array[i].scope;
